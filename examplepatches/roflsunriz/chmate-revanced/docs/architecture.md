...
- JUnit Jupiter `6.1.3` / Platform Launcher `6.1.3`: Kotlin の patch ロジックと Java の runtime ロジックを同じ test engine で検証するため。Gradle 9 は launcher の明示的な runtime 依存を要求する。JUnit 4 は Kotlin / Java 両方で使えるが、今後の parameterized test 拡張性から不採用。
